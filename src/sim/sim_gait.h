...
namespace pluto::sim
{
  ...
  public:
    ...
    void stand(std::array<pluto::sim::Leg, 4>& legs) const noexcept;
    void update(std::array<pluto::sim::Leg, 4>& legs, uint32_t now_ms) const noexcept;
  ...
  private:
    ...
    void write_leg(std::array<pluto::sim::Leg, 4>& legs, LegSide side, float time_s) const noexcept;
    void write_bow(std::array<pluto::sim::Leg, 4>& legs, float time_s) const noexcept;
    void write_paw(std::array<pluto::sim::Leg, 4>& legs, float time_s) const noexcept;
