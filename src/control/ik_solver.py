...
def IK(x, y, z, coxa, femur, tibia):
    ...
    tibia_angle = -np.arccos(n / tibia)

    # Angle for the femur
    femur_angle = -np.arctan(x / D) + np.arccos((femur + n) / G)
...
hip_offsets                                : array of hip coordinate values, namely [xhipf, yhipl] 

returns : the angles of the coxa, femur and tibia of each leg
"""
def robot_IK(xvals, yvals, zvals, coxa, femur, tibia, hip_offsets):
    fl_angles = IK(xvals[0] - hip_offsets[0], yvals[0] - hip_offsets[1], zvals[0], coxa, femur, tibia)
    fr_angles = IK(xvals[1] - hip_offsets[0], yvals[1] + hip_offsets[1], zvals[1], -coxa, femur, tibia)
    bl_angles = IK(xvals[2] - hip_offsets[0], yvals[2] - hip_offsets[1], zvals[2], coxa, femur, tibia)
    br_angles = IK(xvals[3] - hip_offsets[0], yvals[3] + hip_offsets[1], zvals[3], -coxa, femur, tibia)
